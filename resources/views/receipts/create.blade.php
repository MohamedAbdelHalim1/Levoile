...
<div class="p-2">
  <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
    <div class="bg-white shadow sm:rounded-lg p-4">
      <h4 class="mb-3">{{ __('messages.received_quantity') }} : طلب رقم #{{ $req->id }} — {{ $req->material->name }}</h4>
                ...
                <tr>
                  ...
                  <td style="min-width:130px">
                    <select class="form-control" name="items[{{ $index }}][unit]">
                      ...
                      <option value="kg" @selected(old('items.'.$index.'.unit')=='kg')>{{ __('messages.kg') }}</option>
                      <option value="meter" @selected(old('items.'.$index.'.unit')=='meter')>{{ __('messages.meter') }}</option>
                    </select>
                  </td>
                </tr>
              @endforeach
