          ...
          <tbody>
            @forelse($requests as $i => $req)
              <tr>
                ...
                <td>
                  @php $badge = ['open'=>'secondary','partial'=>'warning text-dark','complete'=>'success','cancelled'=>'dark'][$req->status] ?? 'secondary'; @endphp
                  <span class="badge bg-{{ $badge }}">{{ $req->status }}</span>
                </td>
                <td>{{ optional($req->requested_at)->format('Y-m-d H:i') }}</td>
                <td>
                  <a href="{{ route('requests.receive.form', $req->id) }}" class="btn btn-sm btn-primary">استلام</a>
                </td>
              </tr>
              @if($req->items->count())
                <tr>
                      ...
                      <tbody>
                        @foreach($req->items as $it)
                          @php
                            ...
                            $remaining = ($it->required_quantity ?? 0) - $received;
                          @endphp
                          <tr>
                            <td>{{ $it->color->name }}</td>
                            <td>{{ $it->color->code ?? '-' }}</td>
