        ...
        <div class="bg-white shadow sm:rounded-lg p-4">
            ...
            <div class="table-responsive">
                <form method="POST" action="{{ route('ready-to-shoot.start') }}">
                        ...
                        <tbody>
                            ...
                            @foreach ($grouped as $productId => $items)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $product->name }}</td>
                                    <td>
                                        <span class="badge bg-primary" tabindex="0" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-html="true"
                                            data-bs-content="<ul style='margin:0;padding-left:15px;'>@foreach($colorCodes as $code)<li>{{ $code }}</li>@endforeach</ul>">
                                            {{ $colorCodes->count() }}
                                        </span>
                                    </td>
                                    <td>{{ $type ?? '-' }}</td>
                    ...
                    <button type="submit" class="btn btn-success mt-3" id="startShootingBtn" style="display:none;">بدء التصوير</button>
                </form>
            </div>
        </div>
    </div>
